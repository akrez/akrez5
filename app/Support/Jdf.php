...
class Jdf
{
    ...
    public static function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'en')
    {
        ...
        for ($i = 0; $i < $sl; ++$i) {
            ...
            switch ($sub) {
                ...
                case 't':
                    $out .= (12 != $j_m) ? (31 - (int) ($j_m / 6.5)) : ($kab + 29);
                ...
                case 'w':
                    $out .= (6 == $date[7]) ? 0 : $date[7] + 1;

                    break;

                case 'W':
                    $avs = ((6 == $date[7]) ? 0 : $date[7] + 1) - ($doy % 7);
                    if ($avs < 0) {
                        $avs += 7;
                    }
                    $num = (int) (($doy + $avs) / 7);
                    if ($avs < 4) {
                        ...
                        $num = (4 == $avs or $avs == ((((($j_y % 33) % 4) - 2) == ((int) (($j_y % 33) * 0.05))) ? 5 : 4)) ? 53 : 52;
                    }
                    $aks = $avs + $kab;
                    if (7 == $aks) {
                        $aks = 0;
        ...
        return ('en' != $tr_num) ? self::tr_num($out, 'fa', '.') : $out;
    }
    ...
    public static function jcheckdate($jm, $jd, $jy)
    {
        ...
        $l_d = (12 == $jm) ? ((((($jy % 33) % 4) - 1) == ((int) (($jy % 33) * 0.05))) ? 30 : 29) : 31 - (int) ($jm / 6.5);
    ...
    public static function tr_num($str, $mod = 'en', $mf = '٫')
    {
        ...
        return ('fa' == $mod) ? str_replace($num_a, $key_a, $str) : str_replace($key_a, $num_a, $str);
    }
            ...
            switch ($type) {
                case 'ss':
                    ...
                    if (1 == $xy3) {
                        ...
                        $p34 = (0 == $xy3 or 0 == $xy4) ? '' : ' و ';
                        $k3 = ['', '', 'بیست', 'سی', 'چهل', 'پنجاه', 'شصت', 'هفتاد', 'هشتاد', 'نود'];
                        $h3 = $k3[$xy3];
                        $k4 = ['', 'یک', 'دو', 'سه', 'چهار', 'پنج', 'شش', 'هفت', 'هشت', 'نه'];
                        $h4 = $k4[$xy4];
                    }
                    $array[$type] = (($num > 99) ? str_replace(['12', '13', '14', '19', '20'], ['هزار و دویست', 'هزار و سیصد', 'هزار و چهارصد', 'هزار و نهصد', 'دوهزار'], substr($num, 0, 2)) . (('00' == substr($num, 2, 2)) ? '' : ' و ') : '') . $h3 . $p34 . $h34 . $h4;
    ...
    public static function jalali_to_gregorian($jy, $jm, $jd, $mod = '')
    {
        ...
        $gd = $days + 1;
        foreach ([0, 31, (((0 == $gy % 4) and (0 != $gy % 100)) or (0 == $gy % 400)) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] as $gm => $v) {
            if ($gd <= $v) {
                break;
            }
            $gd -= $v;
